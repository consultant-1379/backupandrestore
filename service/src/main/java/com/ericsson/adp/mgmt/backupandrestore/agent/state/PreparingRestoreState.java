...
/**
 * Represents a registered agent who is preparing for a restore.
 */
public class PreparingRestoreState extends RestoreState {
     ...
     */
    public PreparingRestoreState(final Register registrationInformation, final RestoreJob job) {
        super(registrationInformation, job);
    }

    @Override
    public AgentStateChange executeRestore(final AgentInputStream inputStream) {
        return new AgentStateChange(new ExecutingRestoreState(this.registrationInformation, this.job), inputStream::executeRestore);
    }
