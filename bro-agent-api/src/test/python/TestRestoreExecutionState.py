...
def test_restore_execution_state_changes_to_restore_post_action_state():
    restore_execution_state = bro_agent.agent.states.RestoreExecutionState(Mock(), Mock())
    message = generate_post_actions_message()

    new_state = restore_execution_state.process_message(message)

    assert isinstance(new_state, bro_agent.agent.RestorePostActionState.RestorePostActionState)


def test_restore_execution_state_changes_to_cancel_action_state():
    restore_execution_state = bro_agent.agent.states.RestoreExecutionState(Mock(), Mock())
...
def test_restore_execution_state_changes_to_waiting_for_action_state():
    restore_execution_state = bro_agent.agent.states.RestoreExecutionState(Mock(), Mock())
...
def test_restore_execution_state_changes_to_finished_action_state():
    restore_execution_state = bro_agent.agent.states.RestoreExecutionState(Mock(), Mock())

    new_state = restore_execution_state.finish_action()
