...
/**
 * Stage in which agents are executing a Restore.
 */
public class ExecutingRestoreJobStage extends RestoreJobStage {
     ...
     */
    public ExecutingRestoreJobStage(final List<Agent> agents, final RestoreJob job, final NotificationService notificationService) {
        super(agents, job, notificationService);
    }

    @Override
    public void handleTrigger() {
        agents.forEach(Agent::executeRestore);
    }

    @Override
    protected JobStage<RestoreJob> getNextStageSuccess() {
        return new PostActionRestoreJobStage(agents, job, notificationService);
    }
